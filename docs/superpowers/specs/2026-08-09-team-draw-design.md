...
반 학생을 여러 팀/조로 나눌 때 쓰는 범용 기능. 체육 수업뿐 아니라 일반 수업(모둠 활동 등)에서도 사용한다.

- **일반 수업**: 성별/능력과 무관하게 순수 랜덤으로, 또는 성별만 고려해 팀을 나눈다.
- **체육 수업**: 팀 간 실력 편차가 크게 벌어지지 않도록 학생별 "신체능력 등급(상/중/하)"을 추가로 고려해 팀을 자동 배분할 수 있다.

성별 균형·신체능력 균형은 각각 독립적인 체크박스로, 필요할 때만 켜서 쓰는 선택 사항이다(둘 다 끄면 완전 랜덤 배분이 되어 일반 수업에서 그대로 쓸 수 있다). 등급 자체를 입력하지 않은 반(일반 수업)에서는 모든 참가자의 `ability`가 `null`인 채로 동작하며, 이 경우 신체능력 균형 배분 체크박스를 켜더라도 전원이 "미지정" 그룹으로 처리되어 사실상 완전 랜덤과 동일하게 동작한다.
...
- (공개) GitHub 저장소에서 명단을 불러오는 기능 — 이 기능과는 독립적인 별도 기능이며, 이번 스펙 완료 후 별도 스펙/구현으로 다룬다.
